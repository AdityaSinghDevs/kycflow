...
class OCRExtractor:
    ...
    def extract_structured(
        ...
        Returns:
            ...
            {
                "raw_text": [...],
                "document_type": "aadhaar" | "pan" | "driving_license" | etc,
                "fields": {
                    "name": "...",
                    "dob": "...",
                    "document_number": "...",
    ...
    def _parse_fields(self, texts: List[str]) -> Dict[str, str]:
        ...
        # Extract name (usually first significant text line)
        ...
        # Extract address (usually multi-line, near bottom)
    ...
    def _classify_dates(self, texts: List[str], dates: List[str]) -> Dict[str, str]:
        ...
        for i, text in enumerate(texts):
           ...
           # Check for DOI keywords
                ...
                for date in dates:
                     if (i + 1 < len(texts) and date in texts[i + 1]):
...
# If dates found but not classified, assign first as DOB
    ...
    def _extract_name(self, texts: List[str]) -> Optional[str]:
        ...
        """
        for text in texts:
            # Skip short texts and common headers
            if len(text) < 3:
                continue
            ...
            # Check if line contains "Name:" pattern
    ...
    def _extract_parent_names(self, texts: List[str]) -> Dict[str, str]:
        """Extract father's and mother's name."""
        parent_fields = {}
        ...
        for i, text in enumerate(texts):
            ...
            for keyword in self.field_keywords["father_name"]:
                if keyword in text_lower:
                    # Look in current or next line
                    if i + 1 < len(texts):
                        parent_fields["father_name"] = texts[i + 1].strip()
                    break
            ...
            for keyword in self.field_keywords["mother_name"]:
                if keyword in text_lower:
                    if i + 1 < len(texts):
                        parent_fields["mother_name"] = texts[i + 1].strip()
                    break
    ...
    def _extract_gender(self, text: str) -> Optional[str]:
        ...
        if "male" in text_lower and "female" not in text_lower:
            ...
            return "Female"
        elif re.search(r"\bm\b", text_lower):  # Single 'M'
            return "Male"
        elif re.search(r"\bf\b", text_lower):  # Single 'F'
            return "Female"
    ...
    def _extract_address(self, texts: List[str]) -> Optional[str]:
        ...
        for text in texts:
            ...
            if any(kw in text_lower for kw in self.field_keywords["address"]):
                ...
                # Check if address is on same line after keyword
            ...
            if found_address_keyword and len(text) > 10:
                ...
                # Stop if we've collected 3-4 lines
    ...
    def format_output(self, structured_data: Dict[str, Any]) -> str:
        ...
        Args:
            structured_data: Output from extract_structured()
